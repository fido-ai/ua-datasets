...
class NewsClassificationDataset:
	...
	def __init__(self, root: str, download: bool = True, split: str = 'train', return_tags: bool = False):
		""" News Classification Dataset

		Args:
			root (:obj: `str`): direcoty path
	...
	def download(self) -> None:
		...
		text = request.urlopen(self.data_file + self.split + '.csv').read().decode('utf8')
	...
	def labels(self) -> List[str]:
		""" Target names """
		return set([row[self._columns.index('target')] for row in self._data[:-1]])
